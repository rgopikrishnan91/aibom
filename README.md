...
Both SPDX and CycloneDX exports preserve the same fields, the same provenance information, and the same conflict triplets — SPDX through `ai_AIPackage` / `dataset_DatasetPackage` and the SPDX `trainedOn` / `testedOn` / `dependsOn` relationships; CycloneDX through the `modelCard` extension (`modelParameters.task`, `modelParameters.architectureFamily`, `modelParameters.datasets`, `quantitativeAnalysis.performanceMetrics`, `considerations.technicalLimitations`) and `pedigree.ancestors` for lineage. SPDX exports run through the bundled SPDX 3.0.1 JSON Schema by default; `--strict-spdx-validation` adds the official SHACL pass (beta). The same validators apply to the linked SPDX bundle.

**Conflicts in the exported BOMs.** Every detected conflict (direct disagreement between sources, RAG-internal contradiction, or RAG-external disagreement) is materialised in the exported artifacts:

- **SPDX 3.0.1:** one SPDX Core `Annotation` Element inside `@graph` per conflict, with `subject` pointing at the affected `ai_AIPackage` / `dataset_DatasetPackage`, `annotationType: "other"`, `contentType: "application/json"`, and a JSON-encoded `statement` carrying the full provenance (claims by source, narrative, grounding score, confidence band, selected sources). The `name` and `statement.spdx_property` are keyed on the **SPDX 3.0.1 property name** (`ai_safetyRiskAssessment`, `ai_informationAboutTraining`, `simplelicensing_licenseExpression`, `trainedOn`, …), so a downstream consumer can identify the conflicted property without parsing AIkaBoOM's internal field vocabulary. Annotations propagate through the linked recursive bundle and pass strict SHACL.
- **CycloneDX 1.6:** the same content as a component-level `properties` entry per conflict — `name` is `aikaboom:conflict:<spdx_property>`, `value` is the same JSON payload as the SPDX `statement`. (CycloneDX 1.7 introduced a top-level `annotations` array but the validator we ship only embeds schemas through 1.6; properties is the canonical extensibility slot there.)

See [docs/PIPELINE_WALKTHROUGH.md §8.3](docs/PIPELINE_WALKTHROUGH.md#83-conflict-annotations-in-the-emitted-spdx--cyclonedx) for the exact annotation shape and worked examples.
